...
OLS_lineal_log <- lm(formula = log_krig_l ~ M2LOTE + DIST_AGUA  
                + DIST_CLOACAS + DIST_C_SALUD + DIST_ESCUELAS  +
                  ...
                  DIST_AREA_INUNDABLE + 
                  DIST_PARADAS_BUS, data=lineal_log)

#Resumen OLS
summary(OLS_lineal_log) #VER SI SE PUEDE PASAR A EXCEL
#Estadisticos
ols_regress(OLS_lineal_log)
#VIF
ols_vif_tol(OLS_lineal_log)
...
#por otro lado, el analisis VIF muestra la presencia de variables con multicolinealidad. 
...
rev_OLS_lineal_log <- lm(formula = log_krig_l ~ DIST_AGUA  
...
#par(mfrow=c(2,2))
#plot(rev_OLS_lineal_log)

#plot(1:30)

#Resumen OLS
OLS_rev_summary <- summary(rev_OLS_lineal_log)
OLS_rev_summary
#VIF
ols_vif_tol(rev_OLS_lineal_log)
#AIC
ols_aic(rev_OLS_lineal_log)
#estadisticos
ols_regress(rev_OLS_lineal_log)

#linealidad
ols_plot_resid_fit(rev_OLS_lineal_log)
#distribucion normal de los residuales
ols_plot_resid_hist(rev_OLS_lineal_log)

#test estadisticos de normalidad

#la prueba de Shapiro requiere de una muestra entre 3 y 5000
#sampling
sample_model = lineal_log[0:5000, c(1:20)]

sample_rev_OLS_lineal_log <- lm(formula = log_krig_l ~ DIST_AGUA  
                                + DIST_C_SALUD + DIST_ESCUELAS  +
                                  DIST_ESPACIOS_V + DIST_BARRIOS + 
                                  DIST_ESTACIONES +  
                                  DIST_RED_VIAL + 
...
ols_test_normality(sample_rev_OLS_lineal_log)
...
ols_resid<-as.data.frame(rev_OLS_lineal_log$residuals)

sp_ols_resid <- cbind(lineal_log, ols_resid)

tmap_mode("view")

  tm_shape(sp_ols_resid) +  
  tm_dots(col = "rev_OLS_lineal_log.residuals",
          alpha = 0.4,
          style="quantile") +
...
centroid_sp_lineal_log <- st_centroid(sp_ols_resid)
knn_lineal_log <- knn2nb(knearneigh(centroid_sp_lineal_log, k = 5)) 

dist <- nbdists(knn_lineal_log, centroid_sp_lineal_log)
# head(dist1)
summary(unlist(dist)) #min 8.12 y max 1827.27 mts con 5 vecinos

nb <- dnearneigh(x = centroid_sp_lineal_log, d1 = 8.12, d2 = 1827.27, longlat = FALSE)
summary(nb)

nb_lw <- nb2listw(nb, style = "S",zero.policy = TRUE)
summary(nb_lw)

i.moran<-lm.morantest(rev_OLS_lineal_log, nb_lw)
i.moran
#p valor esperado < 2.2e-16 para aceptar la hip nula de residuos aleatorios 
#Valor observado de  2.383 > 0.00000000000000022 por lo tanto rechaza H0